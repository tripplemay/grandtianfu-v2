...
3. **P2：数值输入 Escape 不能撤销已写入的中间值。**
   当前 `NumberField` 对每个有效 `onChange` 立即调用 `onChange` 写入 timeline；Escape 处理器只恢复输入框文本，不恢复模型。例如从 `1500` 输入到 `1600` 后按 Escape，模型仍为 `1600`、状态仍为未保存。若“取消”仅指确认弹窗，该项可降级为输入交互缺陷；否则违反取消不应改变模型状态的预期。该风险尚未修复/复验。
